...
from yolov8.yolo.engine.model import YOLO, TASK_MAP

from yolov8.yolo.utils import SETTINGS, colorstr, ops, is_git_dir, IterableSimpleNamespace
from yolov8.yolo.utils.checks import check_imgsz, print_args
from yolov8.yolo.utils.files import increment_path
from yolov8.yolo.engine.results import Boxes
from yolov8.yolo.data.utils import VID_FORMATS
...
def run(args):
    ...
    for frame_idx, batch in enumerate(predictor.dataset):
        ...
        for i in range(n):
            ...
            predictor.plotted_img = predictor.results[i].plot(
                line_width=1,
                font_size=0.5,
                labels=not predictor.args.hide_label,
                conf=not predictor.args.hide_conf
            )
...
def parse_opt():
    ...
    parser.add_argument('--yolo-model', type=Path, default=WEIGHTS / 'yolov8/yolo/v8/segment/best.pt', help='model.pt path(s)')
    #parser.add_argument('--yolo-model', type=Path, default=WEIGHTS / 'examples/datasets/best.pt', help='model.pt path(s)')
    parser.add_argument('--reid-model', type=Path, default=WEIGHTS / 'resnet50_msmt17_combineall_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip_jitter.pth')
    parser.add_argument('--tracking-method', type=str, default='ocsort', help='deepocsort, botsort, strongsort, ocsort, bytetrack')
    parser.add_argument('--source', type=str, default=r'/Users/momota_yin/Aquaculture/PycharmProjects/Deepocsort_tracking-master/examples/datasets/Z02.mp4', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640], help='inference size h,w')
    parser.add_argument('--conf', type=float, default=0.5, help='confidence threshold')
    parser.add_argument('--iou', type=float, default=0.7, help='intersection over union (IoU) threshold for NMS')
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--show', default="true",action='store_true', help='display tracking video results')
    ...
    parser.add_argument('--save-txt', default="true",action='store_true', help='save tracking results in a txt file')
